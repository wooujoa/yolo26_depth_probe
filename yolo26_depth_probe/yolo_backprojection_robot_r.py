...
class YoloBackprojectionNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("info_topic", "/camera_r/camera_r/aligned_depth_to_color/camera_info")
        ...
        self.declare_parameter("core_min_boundary_px", 3.0)
        ...
        self.core_min_boundary_px = float(self.get_parameter("core_min_boundary_px").value)
        ...
        self.get_logger().info(f"core_min_boundary_px    : {self.core_min_boundary_px}")
        self.get_logger().info(f"core_erode_kernel       : {self.core_erode_kernel}")
        self.get_logger().info(f"core_erode_iterations   : {self.core_erode_iterations}")
        self.get_logger().info(f"safe_dilate_kernel      : {self.safe_dilate_kernel}")
        self.get_logger().info(f"safe_dilate_iterations  : {self.safe_dilate_iterations}")
        self.get_logger().info(f"min_object_core_pixels  : {self.min_object_core_pixels}")
    ...
    def _process_detection_to_pc(self, header, frame_rgb, overlay_bgr, result):
        ...
        if result.masks is not None and result.masks.data is not None:
            try:
                ...
                obj_mask_raw = np.zeros((h, w), dtype=bool)
                for m in masks_np:
                    m_bin = (m > 0.5).astype(np.uint8)
                    if m_bin.shape[0] != h or m_bin.shape[1] != w:
                        m_bin = cv2.resize(m_bin, (w, h), interpolation=cv2.INTER_NEAREST)
                    obj_mask_raw |= (m_bin > 0)
            except Exception as e:
                self.get_logger().warn(f"⚠️ Failed to parse segmentation mask: {e}")
                obj_mask_raw = None

        if obj_mask_raw is None:
            self.get_logger().warn("⚠️ Segmentation mask missing. Falling back to ROI mask.")
            obj_mask_raw = np.array(roi_mask, copy=True)

        obj_mask_raw = obj_mask_raw & roi_mask
        obj_mask_core, obj_mask_safe = self._refine_object_masks(obj_mask_raw, roi_mask)

        depth_ref_mask = obj_mask_core if np.any(obj_mask_core) else obj_mask_raw
        bg_in_roi = roi_mask & (~obj_mask_safe)

        if self.use_depth_band:
            obj_depths = depth_m[depth_ref_mask]
            ...
            if obj_depths.size > 0:
                ...
                obj_mask_raw = obj_mask_raw & band_mask
                obj_mask_core = obj_mask_core & band_mask
                obj_mask_safe = obj_mask_safe & band_mask
        ...
        # grasp 입력은 경계 seam을 줄이기 위해 object core를 우선 사용
        grasp_input_mask = obj_mask_core if np.any(obj_mask_core) else obj_mask_raw
        if not np.any(grasp_input_mask):
            grasp_input_mask = np.array(roi_mask, copy=True)

        self._draw_mask_contours(overlay_bgr, obj_mask_raw, (0, 255, 0))
        self._draw_mask_contours(overlay_bgr, obj_mask_core, (255, 128, 0))
        self._draw_mask_contours(overlay_bgr, bg_in_roi, (0, 0, 255))

        roi_cloud = self._build_cloud_from_mask(header, depth_m, frame_rgb, grasp_input_mask)
        obj_cloud = self._build_cloud_from_mask(header, depth_m, frame_rgb, obj_mask_raw)
        ...
        cv2.putText(
            overlay_bgr,
            f"grasp_input={roi_n} object={obj_n} background={bg_n}",
            (20, 60),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.65,
            (255, 255, 255),
        ...
        self.get_logger().info(f"[pc] grasp_input={roi_n}, object={obj_n}, background={bg_n}")
        ...
        if roi_cloud is not None and roi_n >= self.min_roi_points:
            ...
            self.get_logger().info("📦 Published grasp_input/object/background point clouds")
        else:
            self.get_logger().warn("⚠️ grasp_input point cloud too small, publish skipped")
    ...
    def _make_odd_kernel_size(self, value: int) -> int:
        value = max(1, int(value))
        if value % 2 == 0:
            value += 1
        return value

    def _refine_object_masks(self, obj_mask_raw: np.ndarray, roi_mask: np.ndarray):
        if obj_mask_raw is None:
            empty = np.zeros_like(roi_mask, dtype=bool)
            return empty, empty

        obj_mask_raw = obj_mask_raw.astype(bool) & roi_mask.astype(bool)
        if not np.any(obj_mask_raw):
            empty = np.zeros_like(obj_mask_raw, dtype=bool)
            return empty, empty

        obj_mask_u8 = (obj_mask_raw.astype(np.uint8) * 255)

        dist_px = cv2.distanceTransform(obj_mask_u8, cv2.DIST_L2, 5)
        obj_mask_core = obj_mask_raw & (dist_px >= float(self.core_min_boundary_px))

        if int(np.count_nonzero(obj_mask_core)) < self.min_object_core_pixels:
            erode_k = self._make_odd_kernel_size(self.core_erode_kernel)
            erode_kernel = np.ones((erode_k, erode_k), np.uint8)
            obj_mask_core = cv2.erode(
                obj_mask_u8,
                erode_kernel,
                iterations=max(0, int(self.core_erode_iterations)),
            ) > 0
            obj_mask_core &= obj_mask_raw

        if int(np.count_nonzero(obj_mask_core)) < self.min_object_core_pixels:
            obj_mask_core = np.array(obj_mask_raw, copy=True)

        dilate_k = self._make_odd_kernel_size(self.safe_dilate_kernel)
        dilate_kernel = np.ones((dilate_k, dilate_k), np.uint8)
        obj_mask_safe = cv2.dilate(
            obj_mask_u8,
            dilate_kernel,
            iterations=max(0, int(self.safe_dilate_iterations)),
        ) > 0
        obj_mask_safe &= roi_mask

        return obj_mask_core.astype(bool), obj_mask_safe.astype(bool)

    def _draw_mask_contours(self, overlay_bgr, mask, color):
